...
export class BaseItem {
    ...
    private handlePickupCollision(other: BlockType | Entity, started: boolean) {
        ...
        try {
            ...
            // Ensure we have the most up-to-date durability info before adding the item
            ...
            if (result.success && result.addedToSlot !== undefined) {
                // If the item was added to the selected slot and it's different from what was there before
                // This is now handled in PlayerInventory.addItemWithInstance, but we still need to show the name
                if (result.addedToSlot === selectedSlot) {
                    const displayName = this.itemConfig.displayName || this.itemType
                        .split('-')
                        .map(word => word.charAt(0).toUpperCase() + word.slice(1))
                        .join(' ');
                    
                    // Send durability information if the item has it
